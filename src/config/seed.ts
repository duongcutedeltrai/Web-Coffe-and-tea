...
const initDatabase = async () => {
  ...
  if (countOrder == 0) {
    const customer = await prisma.users.findFirst({
      where: { email: "duonghaitt311@gmail.com" },
    });
    ...
    if (customer && products.length > 0) {
      for (let i = 0; i < 2; i++) {
        // Chọn ngẫu nhiên 3 sản phẩm cho mỗi đơn hàng
        const chosenProducts = products
          .sort(() => 0.5 - Math.random())
          .slice(0, 3);

        // Tạo order trước (số tiền tạm thời = 0)
        const order = await prisma.orders.create({
          data: {
            order_id: `ORD-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
            user_id: customer.user_id,
            delivery_address: "Hà Nội",
            receiver_name: "Phạm Anh Dương",
            receiver_phone: "0987654321",
            status: orders_status.pending,

            order_type: i % 2 === 0 ? OrderType.DINE_IN : OrderType.TAKE_AWAY,
            order_source:
              i % 2 === 0 ? OrderSource.STAFF : OrderSource.CUSTOMER,
            original_amount: 0,
            discount_amount: 0,
            final_amount: 0,
          },
        ...
        for (const prod of chosenProducts) {
          const price = prod.price_product[0]?.price ?? 0; // lấy giá size đầu tiên
        ...
        await prisma.orders.update({
          ...
          data: {
            ...
            final_amount: total, // chưa có khuyến mãi
          },
        });
  ...
  if (countProduct === 0) {
    const products = [
      ...
      {
        name: "TẤM",
        description: "Trà Ô Long đậm đà kết hợp hài hoà với gạo rang thơm bùi.",
      ...
      },
      {
        ...
        description: "Hương vị cà phê nguyên bản qua phương pháp phin giấy.",
        price: 442000,
        category_id: 10,
