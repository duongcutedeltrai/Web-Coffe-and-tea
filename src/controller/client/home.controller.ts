...
class ClientHomeController {
  ...
  getProductDetailPage = async (req: Request, res: Response) => {
    const product = await productsService.getDetailProductsById(+req.params.id);
  ...
  getProfileClientPage = async (req: Request, res: Response) => {
    const user = await UserService.getDetailCustomerById(
      +(req.user as any)?.id || 0
    );
    return res.render("client/profile/profile.ejs", { user });
  };

  getDataOfUser = async (req: Request, res: Response) => {
    const user = await UserService.getDetailCustomerById(
      +(req.user as any)?.id || 0
    );
  ...
  updateProfileClient = async (req: Request, res: Response) => {
    console.log("Update profile data:", req.body);
    const { name, phone, address } = req.body;
    const updatedUser = await clientService.updateCustomerProfile(
      +(req.user as any)?.id || 0,
      name,
      phone,
      address
  ...
  uploadAvatarClient = async (req: Request, res: Response) => {
    try {
      if (!req.file) {
      ...
      const result = await cloudinary.uploader.upload(req.file.path, {
        folder: "avatars",
        public_id: `user_${(req.user as any)?.id}_${Date.now()}`,
        resource_type: "auto",
      });
      ...
      // Cập nhật database
      const updatedUser = await clientService.updateCustomerAvatar(
        +(req.user as any)?.id || 0,
        result.secure_url
      );
