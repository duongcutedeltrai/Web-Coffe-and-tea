...
class AdminStatisticService {
    getStatisticPage = async (period: string) => {
        ...
        if (period === "week") {
            for (let i = count - 1; i >= 0; i--) {
                ...
                const ordersData = await prisma.orders.findMany({
                    ...
                    select: { total_amount: true, order_id: true },
                });
        ...
        let thisMonthOrderIds = await getOrderIdsInRange(startThisMonth, endThisMonth);
        let lastMonthOrderIds = await getOrderIdsInRange(startLastMonth, endLastMonth);
        ...
        if (thisMonthOrderIds.length === 0) {
            ...
            thisMonthOrderIds = await getOrderIdsInRange(startLastMonth, endLastMonth);
            lastMonthOrderIds = await getOrderIdsInRange(startPrevPrev, endPrevPrev);
        }
        ...
        const trendingRaw: { product_id: number; sold: number; growth: number }[] = [];
        ...
        // sort & top 5
        const topTrending = trendingRaw.sort((a, b) => b.growth - a.growth).slice(0, 5);
        ...
        const trendingProducts = topTrending.map((t) => {
            const prod = trendingInfo.find((p) => p.product_id === t.product_id);
            return {
        ...
        if (validFavorites.length > 0) {
            ...
            favoriteProducts = validFavorites.map((f) => {
                const prod = favoriteInfo.find((p) => p.product_id === f.product_id);
                return {
