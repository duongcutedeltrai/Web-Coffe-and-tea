...
function renderProductSelection() {
  ...
  grid.innerHTML = filteredProducts
    .map((product) => {
      ...
      const displayPrice = selectedPriceItem
        ...
        <div class="product-card-sizes">
            ...
            .map(
              (priceItem) => `
                  <button 
                    type="button"
                    class="size-btn ${selectedSize === priceItem.size ? "active" : ""
                }"
            ...
            .join("")}
                 ...
                 <button 
                    type="button"
                    class="size-btn size-btn-all ${selectedSize === "all" ? "active" : ""
          }"
                    onclick="event.stopPropagation(); selectProductSize(${product.product_id
          }, 'all', ${displayPrice})"
                   ${!isSelected ? "disabled" : ""}
...
function getProductNames(applicableProducts) {
  ...
  if (names.length > 3) {
    return `${names.slice(0, 3).join(", ")} và ${names.length - 3
      } sản phẩm khác`;
  }
        ...
        .map((p) => {
          ...
          return `
            ...
            discountAmount
              ...
              <td class="cell-quantity">${item.current_usage}/${item.max_usage_count || 0
            }</td>
              <td class="cell-discount-amount">${formatCurrency(
              discountPrice
            )}</td>
              <td class="cell-actions" onclick="event.stopPropagation()">
                <button class="btn-action btn-edit" onclick="editFlashsale('${item.promotion_id || index
            }')">Sửa</button>
                <button class="btn-action btn-delete" onclick="deleteFlashsale('${item.promotion_id || index
            }')">Xóa</button>
              </td>
            </tr>
          `;
  ...
  container.innerHTML = filtered
    .map((promotion) => {
      ...
      const membershipBadges = promotion.applicable_membership
        .map(
          (m) => `<span class="badge">${getMembershipText(m)}</span>`
        )
        .join("");
      ...
      return `
            <div class="promotion-card" onclick="viewPromotionDetails('${promotion.promotion_id
        }')">
          ...
          status
                ...
                <div class="promotion-description">${promotion.description
        }</div>
                <div class="promotion-details">
                    <div class="detail-item">
                        <span class="detail-label">Giảm giá</span>
                        <span class="detail-value">${promotion.discount_percent
        }%</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Đơn tối thiểu</span>
                        <span class="detail-value">${promotion.min_order_amount
          ...
          : "Không"
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value">${promotion.current_usage}/${promotion.max_usage_count || "∞"
        }</span>
                    </div>
                    <div class="detail-item">
          ...
          : ""
                ...
                <div class="promotion-actions" onclick="event.stopPropagation()">
                   <button class="btn-edit" onclick="editPromotion('${promotion.promotion_id
        }')">Sửa</button>
                    <button class="btn-delete" onclick="deletePromotion('${promotion.promotion_id
        }')"> Xóa</button>
                </div>
            </div>
        `;
...
function closePromotionDrawer() {
  document
    .getElementById("promotionDrawerOverlay")
    .classList.remove("active");
...
function editPromotion(id) {
  ...
  Array.from(membershipSelect.options).forEach((option) => {
    option.selected = promotion.applicable_membership.includes(
      option.value
    );
  });
        ...
        <div class="detail-section">
            ...
            <div class="detail-grid">
                <div class="detail-row">
                    ...
                    <span style="color: #b22830; font-family: 'Courier New', monospace; font-size: 18px;">${promotion.code
    }</span>
                </div>
                <div class="detail-row">
    ...
    align-items: center;"> <h3>Điều kiện áp dụng</h3> </div>
            ...
            <div class="detail-grid">
                <div class="detail-row">
                    ...
                    <span style="color: #b22830; font-size: 20px; font-weight: 600;">${promotion.discount_percent
    }%</span>
                </div>
                <div class="detail-row">
                    <label>Giá trị đơn hàng tối thiểu</label>
                    <span>${promotion.min_order_amount
      ? formatCurrency(promotion.min_order_amount)
      : "Không yêu cầu"
    }</span>
                </div>
                <div class="detail-row">
                    ...
                    <span>${promotion.max_usage_count || "Không giới hạn"
    }</span>
                </div>
                <div class="detail-row">
...
async function submitPromotions() {
  ...
  const promotionData = {
    ...
    min_order_amount:
      ...
      0,
    max_usage_count:
      Number.parseInt(document.getElementById("maxUsageCount").value) ||
      null,
    current_usage:
