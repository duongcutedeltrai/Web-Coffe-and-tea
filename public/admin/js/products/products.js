...
(() => {
  ...
  function fetchProducts(page = 1, categoryId = null) {
    ...
    $.ajax({
      ...
      error(err) {
        ...
        $(".loading-wrapper").html("");
        $(".menu-grid").html(`<p class="empty">Không thể tải dữ liệu.</p>`);
      },
    });
  }
  ...
  function setupEventListeners() {
    $(document).on("click", ".category-image img", function () {
      ...
      const categoryId = $(this).closest(".category-item").data("category");
  ...
  function initScrollNavigation() {
    checkScrollButtons();
    $scrollLeft.on("click", () =>
      $categoryNav.animate({ scrollLeft: $categoryNav.scrollLeft() - 250 }, 400)
    );
