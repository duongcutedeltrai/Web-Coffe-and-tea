...
function createOrderCard(order) {
    ...
    const firstThree = items.slice(0, MAX_ITEMS);
    const hiddenItems = items.slice(MAX_ITEMS);
    console.log(order);
    // Get order items from order_details
    const itemsHTML = firstThree
        .map(
            (item) => `
    <div class="order-item">
    <div class="order_item_image"><img src="/images/products/${item.products.images}" style="width: 60px; height: 60px; object-fit: cover;  border-radius: 50%;" alt="">
      <span>${item.quantity}x${item?.
                    size} | ${item.products?.name || "Sản phẩm"}</span></div>
  ...
  `
    ...
    const hiddenHTML = hiddenItems
        .map(
            (item) => `
        <div class="order-item hidden-item" style="display:none;">
            <div class="order_item_image">
                <img src="/images/products/${item.products.images}"
                    style="width: 60px; height: 60px; object-fit: cover; border-radius: 50%;" 
                />
                <span>${item.quantity} x ${item.products?.
                    price_product?.size}|${item.products?.name || "Sản phẩm"}</span>
            </div>
            <span>${formatCurrency(item.price * item.quantity)}</span>
        </div>
    `
    ...
    const showMoreHTML =
        items.length > MAX_ITEMS
            ? `<div class="show-more" style="color:#714024; cursor:pointer;">Xem thêm (${items.length - MAX_ITEMS} sản phẩm)</div>`
            : "";
    // Get customer name
    const customerName =
...
async function renderOrders(tabName = "all", orderType = "") {
    ...
    const res = await fetch(`/admin/data/orders-user?${params.toString()}`);
...
async function viewOrderDetails(orderId) {
    ...
    if (window.location.pathname !== `/orders`) {
        history.pushState({ orderId }, "", `/orders`);
    }
          ...
          <div class="modal-section">
            ...
            <div class="modal-info-grid">
              <div class="modal-info-item">
                ...
                <span class="modal-info-value" id="modalCustomerName">${order.data.receiver_name
        }</span>
              </div>
              <div class="modal-info-item">
                <span class="modal-info-label">Số điện thoại:</span>
                <span class="modal-info-value" id="modalPhone">${order.data.receiver_phone
        }</span>
              </div>
              <div class="modal-info-item full-width">
                <span class="modal-info-label">Địa chỉ giao hàng:</span>
                <span class="modal-info-value" id="modalAddress">${order.data.delivery_address
        }</span>
              </div>
            </div>
          </div>
          ...
          <div class="modal-section">
            ...
            <div class="modal-info-grid">
              <div class="modal-info-item">
                ...
                <span class="modal-info-value" id="modalOrderId">${order.data.order_id
        }</span>
              </div>
              <div class="modal-info-item">
            ...
            order.data.orderDate
              ...
              <div class="modal-info-item">
                ...
                <span class="modal-info-value" id="modalType">${order.data.order_type
        }</span>
              </div>
              <div class="modal-info-item">
            ...
            order.data.status
              ...
              <div class="modal-info-item">
                <span class="modal-info-label">Giảm giá:</span>
                <span class="modal-info-value" id="modalType">${((order.data.original_amount - order.data.final_amount) / order.data.original_amount * 100).toFixed(0)}%
        </span>
              </div>
            </div>
          </div>
...
function renderProductsGrid() {
  ...
  ${product.price_product
        ...
        <button 
          ...
          data-product-id="${product.product_id}"
          onclick="event.stopPropagation(); selectProductSize(${product.product_id
    ...
    orderTypeRadios.forEach((radio) => {
        radio.addEventListener("change", (e) => {
            const tableNumberGroup =
                document.getElementById("tableNumberGroup");
            if (tableNumberGroup) {
...
function selectProductSize(productId, size) {
    ...
    document
        .querySelectorAll(
            `.product-card[data-product-id="${productId}"] .size-btn`
        )
      ...
      ${selectedProducts
        ...
        <div class="selected-product-row">
          ...
          <div class="selected-product-quantity">
            <button type="button" class="quantity-btn" onclick="decreaseQuantity(${product.product_id
                    }, '${product.size}') " ${product.quantity <= 1 ? "disabled" : ""}>
              −
            </button>
            <span class="quantity-value">${product.quantity}</span>
            <button type="button" class="quantity-btn" onclick="increaseQuantity(${product.product_id
                    }, '${product.size}')">
              +
            </button>
          </div>
          <div class="selected-product-actions">
            <button type="button" class="btn-remove-selected" onclick="removeSelectedProduct(${product.product_id
...
async function submitCreateOrder() {
    ...
    if (selectedProducts.promotion) {
        ...
        orderData.discount_percent = selectedProducts.promotion.discountPercent;
    }
...
if (validTabs.includes(statusParam)) {
    ...
    document.querySelectorAll(".tab-trigger").forEach((t) => {
        ...
        if (t.getAttribute("data-tab") === statusParam)
